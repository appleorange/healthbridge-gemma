  ...
  // ── STEP 7: Current coverage ────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          { value: 'parent_employer', label: "A parent's employer plan", description: "Covered as a dependent on a parent's work insurance" },
          { value: 'spouse_employer', label: "A spouse's or partner's plan", description: "Covered as a dependent on a partner's work insurance" },
          { value: 'employer', label: 'My own employer plan', description: 'Enrolled in my own employer coverage' },
      ...
      },
      {
        ...
        required: false,
        showWhen: { field: 'currentPlanType', value: 'parent_employer' },
        helpText: "Check the front of the insurance card.",
      },
      {
        id: 'parentPlanType',
        ...
        showWhen: { field: 'currentPlanType', value: 'parent_employer' },
        options: [
          { value: 'ppo', label: 'PPO', description: 'Can see most doctors without a referral' },
          { value: 'hmo', label: 'HMO', description: 'Must use network doctors, need referrals' },
          { value: 'hdhp', label: 'HDHP (High Deductible)', description: 'Lower premium, higher deductible' },
          { value: 'epo', label: 'EPO', description: 'Network-only but no referrals needed' },
          { value: 'unknown', label: 'Not sure' },
        ],
      },
      {
        id: 'parentPlanPremiumContribution',
        label: "How much do you pay toward this plan each month?",
        type: 'select',
        required: false,
        showWhen: { field: 'currentPlanType', value: 'parent_employer' },
        helpText: "Many parents cover dependents at no extra cost to the dependent.",
        options: [
          { value: '0', label: "$0 — fully covered by parent's employer" },
      ...
      },
      {
        ...
        label: "Are you happy with this coverage?",
        type: 'select',
        required: false,
        showWhen: { field: 'currentPlanType', value: 'parent_employer' },
        options: [
          { value: 'very_happy', label: 'Yes — good coverage, no issues', description: 'My doctors are covered and costs are reasonable' },
          { value: 'somewhat_happy', label: 'Somewhat — a few things missing', description: 'Some gaps or out-of-network issues' },
      ...
      },
      {
        ...
        required: false,
        showWhen: { field: 'currentPlanType', value: 'parent_employer' },
        helpText: "Under ACA rules, you can stay on a parent's plan until age 26 regardless of student or marital status.",
        options: [
          { value: 'over_2_years', label: 'More than 2 years away' },
          { value: '1_to_2_years', label: '1–2 years away' },
          { value: 'under_1_year', label: 'Less than 1 year away — need to plan transition soon' },
