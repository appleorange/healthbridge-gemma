...
export const ONBOARDING_STEPS: OnboardingStep[] = [
  ...
  // ── STEP 3: Employment ──────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      {
        id: 'dependentOnWhom',
        label: "Whose plan are you on?",
        type: 'select',
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        options: [
          { value: 'parent', label: "A parent's plan", description: "You're covered under your parent's or guardian's insurance" },
          { value: 'spouse', label: "A spouse's plan", description: "You're covered under your spouse's employer or marketplace plan" },
          { value: 'partner', label: "A domestic partner's plan", description: "You're covered under a domestic partner's plan" },
        ],
      },
      {
        id: 'parentPlanInsurer',
        label: "What insurance company is the plan through?",
        type: 'text',
        placeholder: 'e.g. Aetna, Blue Cross Blue Shield, UnitedHealth, Cigna',
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        helpText: "Check the insurance card — the insurer name is usually on the front.",
      },
      {
        id: 'parentPlanType',
        ...
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        helpText: "Check your insurance card or ask the plan holder. HMO restricts you to a network, PPO gives more flexibility.",
        options: [
          { value: 'hmo', label: 'HMO', description: 'Must use doctors in the network, need referrals for specialists' },
          { value: 'ppo', label: 'PPO', description: 'More flexibility, can see any doctor, no referral needed' },
          { value: 'hdhp', label: 'HDHP (High Deductible)', description: 'Lower premium, higher deductible, often paired with HSA' },
          { value: 'epo', label: 'EPO', description: 'Network-only like HMO but no referrals needed' },
          { value: 'unknown', label: 'Not sure', description: "I don't know the plan type" },
        ],
      },
      {
        id: 'parentPlanDeductible',
        label: "What is the deductible for your coverage under this plan?",
        type: 'select',
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        helpText: "The deductible is what you pay before insurance kicks in. Check your plan documents or ask the plan holder.",
        options: [
          { value: 'under_500', label: 'Under $500' },
          { value: '500_to_1500', label: '$500 – $1,500' },
          { value: '1500_to_3000', label: '$1,500 – $3,000' },
          { value: 'over_3000', label: 'Over $3,000' },
          { value: 'unknown', label: 'Not sure' },
        ],
      },
      {
        id: 'parentPlanPremiumContribution',
        label: "How much (if anything) do you personally pay toward this plan per month?",
        type: 'select',
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        helpText: "Some employer plans charge extra for adding a dependent. If you pay nothing, select $0.",
        options: [
          { value: '0', label: '$0 — fully covered by parent/spouse employer' },
          { value: 'under_100', label: 'Under $100/month' },
          { value: '100_to_300', label: '$100 – $300/month' },
          { value: 'over_300', label: 'Over $300/month' },
          { value: 'unknown', label: 'Not sure' },
        ],
      },
      {
        id: 'parentPlanSatisfied',
        label: "Are you generally happy with your current coverage?",
        type: 'select',
        required: false,
        showWhen: { field: 'employmentStatus', value: 'dependent' },
        options: [
          { value: 'very_happy', label: 'Yes — good coverage, no issues', description: 'My doctors are covered, costs are reasonable' },
          { value: 'somewhat_happy', label: 'Somewhat — a few things missing', description: 'Missing some benefits or my doctors are out of network' },
          { value: 'unhappy', label: 'No — looking for something better', description: 'Coverage gaps, high costs, or losing this plan soon' },
        ],
      },
      {
        id: 'agingOffDate',
        label: "Do you know when you age off this plan?",
        type: 'select',
        required: false,
        showWhen: { field: 'dependentOnWhom', value: 'parent' },
        helpText: "Under the ACA, you can stay on a parent's plan until age 26 regardless of student or marital status.",
        options: [
          { value: 'over_2_years', label: 'More than 2 years away' },
          { value: '1_to_2_years', label: '1–2 years away' },
          { value: 'under_1_year', label: 'Less than 1 year away — planning transition' },
          { value: 'already_aged_off', label: 'Already aged off or about to' },
          { value: 'unknown', label: 'Not sure' },
        ],
      },
    ],
