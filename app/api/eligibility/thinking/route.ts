...
import { chatWithThinking } from '@/lib/ai/client'
...
function buildVisaPrompt(profile: UserProfile, result: EligibilityResult, language: 'en' | 'es'): string {
  ...
  return `You are a health insurance navigator writing a plain-language explanation for a specific user.
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        const summary = await chatWithThinking(
          [{ role: 'user', content: buildVisaPrompt(parsed.data.profile, result, parsed.data.language ?? 'en') }],
          '',
          (step) => {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'step', step })}\n\n`))
          },
        )
        result.visaEligibilitySummary = summary
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'result', eligibility: result })}\n\n`))
      } catch (err) {
